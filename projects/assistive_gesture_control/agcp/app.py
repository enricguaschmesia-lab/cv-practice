...
from .config import load_assistive_config
from .detector import HandDetector
from .inference import extract_features, predict_gesture
from .recording import GestureRecorder
from .state_machine import GestureStateMachine
from .telemetry import TelemetryLogger
from .types import GestureFeatures
...
def _draw_debug_panel(frame, prediction, profile, machine_debug: dict[str, float | int | bool | str | None]) -> None:
    ...
    lines: list[tuple[str, tuple[int, int, int]]] = [
        ...
        (
            f"pinch_session_active={bool(machine_debug.get('pinch_session_active', False))}",
            (215, 215, 215),
        ),
        ("", (215, 215, 215)),
        ...
        (
            (
                f"two_finger_ratio: {feat.two_finger_ratio:.3f} "
                f"(<= tap_max+tol {(profile.two_finger_tap_max + 0.025):.3f})"
            ),
            (230, 230, 230),
        ),
        (
    ...
    lines.extend(
        [
            ...
            (
                f"unlock/lock conf>=gesture_required (~0.70) hold>={profile.hold_ms}ms",
                (220, 220, 220),
            ),
            (
                (
            ...
            (
                (
                    f"pinch start conf>=gesture_required hold>={profile.hold_ms}ms, "
                    "then session until pose-break"
                ),
                (220, 220, 220),
            ),
            (
