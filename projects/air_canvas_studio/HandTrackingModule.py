...
Model file:
  - models/hand_landmarker.task (inside this project folder)
"""
...
DEFAULT_MODEL_PATH = str(Path(__file__).resolve().parent / "models" / "hand_landmarker.task")
...
class HandDetector:
    ...
    Typical usage:
        ...
        with HandDetector(model_path=DEFAULT_MODEL_PATH) as detector:
    ...
    def __init__(
        ...
        if not os.path.exists(self.model_path):
            raise FileNotFoundError(
                ...
                f"Expected a .task file (e.g., {DEFAULT_MODEL_PATH})."
            )
...
def main() -> None:
    ...
    with HandDetector(model_path=DEFAULT_MODEL_PATH, num_hands=2) as detector:
        while True:
            ok, frame = cap.read()
            if not ok:
                break
